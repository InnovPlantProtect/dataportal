...
#      inpp_metadata_point_of_contact
#      inpp_responsible_organisation_name
#      inpp_contact_mail_address
#      inpp_responsible_party_role
...
# the extra metadata required for INPP. It only needs to be run
...
cat $file_input | while IFS="	" read label description; do
	# Convert the label into an identifier, no spaces, lowercase, inpp_ prefix
	ident=`echo $label | sed -e 's/^/inpp_/' -e 's/ /_/g' | tr '[:upper:]' '[:lower:]'`
	#echo "$ident = $label"
...
sed -i.bak '/INPPSCHEMA_UPDATE_START/,/INPPSCHEMA_UPDATE_END/!b;//!d;/INPPSCHEMA_UPDATE_START/r '${file_plugin_update} ${file_plugin}
sed -i.bck '/INPPSCHEMA_SHOW_START/,/INPPSCHEMA_SHOW_END/!b;//!d;/INPPSCHEMA_SHOW_START/r       '${file_plugin_show}   ${file_plugin}
...
read_options_file_for_additional_info()
{
	...
	options_id="$3"            # The HTML form name, eg. inpp_topic_category
	...
	cat "${options_input_file}" | while IFS="	" read label description; do
		...
		# {% if pkg_dict.inpp_use_constraints == "openbut" %} Open, but something...
        # {% elif pkg_dict.inpp_use_constraints == "restricted" %} Restricted...
        # {% else %}
        # {{ pkg_dict.inpp_use_constraints }}
        # {% endif %}
...
cat $file_input | while IFS="	" read label description; do
	...
	if [ $ident == "inpp_metadata_point_of_contact" \
		-o $ident == "inpp_responsible_organisation_name" \
		-o $ident == "inpp_contact_mail_address" \
		-o $ident == "inpp_responsible_party_role" ]; then
		echo '      {% if (c.userobj and c.userobj.sysadmin) or ( pkg_dict.inpp_contact_consent and pkg_dict.inpp_contact_consent == "1" ) %}{{ pkg_dict.'${ident}' }}'   >> ${file_addinfo_update}
	...
	elif [ "$ident" == "inpp_original_title" ]; then
...
sed -i.bak '/INPPSCHEMA_ADDINFO_START/,/INPPSCHEMA_ADDINFO_END/!b;//!d;/INPPSCHEMA_ADDINFO_START/r '${file_addinfo_update} ${file_addinfo}
...
read_options_file_for_package_basic_fields()
{
	...
	cat "${options_input_file}" | while IFS="	" read label description; do
		...
		# <option value="done" {{ "selected " if "done" == data['inpp_status'] }} >Status</option>
...
sed -i.bak '/INPPSCHEMA_BASICFIELDS_START/,/INPPSCHEMA_BASICFIELDS_END/!b;//!d;/INPPSCHEMA_BASICFIELDS_START/r '${file_basicfields_update} ${file_basicfields}
