#!/usr/bin/env python
# 1.04 arb Sat 23 Mar 01:57:23 GMT 2019 - added inppckan_map_topic_category_to_group
...
# Functions for INPP's CKAN installation
...
# This will be used in ckan_add_dataset.py, and in inppschema plugin
# when updating a dataset to add it to the appropriate groups based on topic_category.
#group_csv_filename="../../../../ckanext-inpptheme/ckanext/inpptheme/tools/topic_categories.csv"
group_csv_filename="/usr/lib/ckan/default/src/ckanext-inpptheme/ckanext/inpptheme/tools/topic_categories.csv"
...
def inppckan_convert_latlon_bbox_to_geojson(n,s,w,e):
...
def inppckan_convert_bbox_to_geojson(srs,top,bottom,left,right):
    src_proj = None

    log.debug("inppckan_convert_bbox_to_geojson SRS %s" % (srs))
    log.debug("inppckan_convert_bbox_to_geojson N %s" % (top))
    log.debug("inppckan_convert_bbox_to_geojson S %s" % (bottom))
    log.debug("inppckan_convert_bbox_to_geojson W %s" % (left))
    log.debug("inppckan_convert_bbox_to_geojson E %s" % (right))
    ...
    try:
        ...
        log.error("inppckan_convert_bbox_to_geojson: ERROR in digits %s %s %s %s %s" % (srs,top,bottom,left,right))
        return ''

    # All zeros is OK:
    if top == 0 and bottom == 0 and left == 0 and right == 0:
        log.debug("inppckan_convert_bbox_to_geojson all zero")
        return ''
    ...
    if srs == 'WGS84' or srs == 'WGS84 ':
        ...
        if top < -90 or top > 90 or bottom < -90 or bottom > 90:
            log.error("inppckan_convert_bbox_to_geojson ERROR out of range1 in %s %s %s %s %s" % (srs,top,bottom,left,right))
            return ''
        if left < -180 or left > 180 or right < -180 or right > 180:
            log.error("inppckan_convert_bbox_to_geojson ERROR out of range2 in %s %s %s %s %s" % (srs,top,bottom,left,right))
            return ''
        return inppckan_convert_latlon_bbox_to_geojson(top,bottom,left,right)
    elif srs == 'ETRS89 TM06': # main for Portugal
        src_proj = Proj(init='epsg:3763')
    elif srs == 'ETRS89 UTM 29N': # utm for portugal
        src_proj = Proj(init='epsg:25829')
    elif srs == 'World Pseudo Mercator WGS84 Datum': # for worldwide datasets
        src_proj = Proj(init='epsg:3857')
    else:
        log.error("inppckan_convert_bbox_to_geojson ERROR unknown SRS %s" % srs)
        return ''
    ...
    if src_proj:
        try:
            ...
            if tl_lat < -90 or tl_lat > 90:
                log.error("inppckan_convert_bbox_to_geojson ERROR %s >90 in %s %s %s %s %s" % (tl_lat,srs,top,bottom,left,right))
                return ''
            if br_lat < -90 or br_lat > 90:
                log.error("inppckan_convert_bbox_to_geojson ERROR %s >90 in %s %s %s %s %s" % (br_lat,srs,top,bottom,left,right))
                return ''
            if tl_lon < -180 or tl_lon > 180:
                log.error("inppckan_convert_bbox_to_geojson ERROR %s >180 in %s %s %s %s %s" % (tl_lon,srs,top,bottom,left,right))
                return ''
            if br_lon < -180 or br_lon > 180:
                log.error("inppckan_convert_bbox_to_geojson ERROR %s >180 in %s %s %s %s %s" % (br_lon,srs,top,bottom,left,right))
                return ''
            if tl_lon == 0 or tl_lat == 0 or br_lon == 0 or br_lat == 0:
                log.error("inppckan_convert_bbox_to_geojson ERROR zero(s) in %s %s %s %s %s" % (srs,top,bottom,left,right))
                return ''
            # Convert to GeoJSON
            geojson = inppckan_convert_latlon_bbox_to_geojson(tl_lat, br_lat, tl_lon, br_lon)
            log.debug("inppckan_convert_bbox_to_geojson GeoJSON %s" % (geojson))
            return geojson
        except:
            # The Proj call failed to transform the coordinates
            log.debug("inppckan_convert_bbox_to_geojson Proj failed to transform coordinates")
            return ''

    # No projection could be identified
    log.debug("inppckan_convert_bbox_to_geojson projection could not be identified")
    return ''
...
# XXX this code also exists inside inpptheme/ckan_add_dataset.py

# We keep a 'global' copy so that we don't have to keep reading the CSV file
topic_category_to_group_dict = {}

def inppckan_create_mapping_topic_category_to_group():
...
def inppckan_map_topic_category_to_group(context, topic_category):
    # Read CSV from theme plugin
    topic_category_to_group_dict = inppckan_create_mapping_topic_category_to_group()
...
def inppckan_name_from_title(title):
