...
csv_filename = "metadata_export.csv"
...
map_CSV_to_CKAN = {
	...
	'keywords': 'tag_string',
	'region':   'inpp_region',
	'language': 'inpp_language',
	'topic_category':  'inpp_topic_category',
	'temporal_extent_start':  'inpp_temporal_extent_start',
	'temporal_extent_end':    'inpp_temporal_extent_end',
	'dataset_reference_date': 'inpp_dataset_reference_date',
	'lineage': 'inpp_lineage',
	'w_long':  'inpp_west_longitude',
	's_lat':   'inpp_south_latitude',
	'e_long':  'inpp_east_longitude',
	'n_lat':   'inpp_north_latitude',
	'spatial_reference_system': 'inpp_spatial_reference_system',
	'responsible_organisation_name': 'inpp_responsible_organisation_name',
	'contact_mail_address':   'inpp_contact_mail_address',
	'responsible_party_role': 'inpp_responsible_party_role',
	'limitations_access':  'inpp_access_limitations',
	'use_constraints':     'inpp_use_constraints',
	'resource_reference':  'inpp_resource_reference',
	'data_format':      'inpp_data_format',
	'frequency_update': 'inpp_update_frequency',
	'accuracy':         'inpp_accuracy',
	'resource_type':    'inpp_resource_type',
	'original_title':   'inpp_original_title',
	'metadata_date':    'inpp_metadata_date',
	'metadata_point_contact': 'inpp_metadata_point_of_contact',
	'unique_resource_id':     'inpp_unique_resource_id',
	'status':       'inpp_status',
	...
	'contact_consent':  'inpp_contact_consent',
}

# Map from the CSV use_constraints to CKAN license_id
map_inpp_use_constraints_to_license_id = {
...
# -----------------------------------------------------------------------
# This function has been moved into inppckan.py
...
def convert_inpp_access_limitations(inpp_access_limitations):
	global map_csv_to_access_limitation

	inpp_access_limitations = inpp_access_limitations.lower().strip()
	...
	if inpp_access_limitations in map_csv_to_access_limitation:
		return map_csv_to_access_limitation[inpp_access_limitations]
	else:
		print("WARNING: access limitation unknown: %s" % inpp_access_limitations)

	# If the input values don't have a one-to-one match we have to guess:
	if 'open access' in inpp_access_limitations:
		return 'open'
	if 'military' in inpp_access_limitations:
		return 'mil'
	if 'commerc' in inpp_access_limitations:
		return 'com-dept'
	if 'environ' in inpp_access_limitations:
		return 'env-dept'
...
def convert_inpp_use_constraints(inpp_use_constraints):
	inpp_use_constraints = inpp_use_constraints.lower()
	...
	if 'open acces' in inpp_use_constraints:
		return 'open'
	if 'restricted' in inpp_use_constraints:
...
def ckan_add_dataset_from_csv_dict(row, dataset_name_to_be_updated):
	...
	dataset_dict['name'] = inppckan.inppckan_name_from_title(dataset_dict['title'])
	...
	if not 'inpp_contact_consent' in dataset_dict:
		dataset_dict['inpp_contact_consent'] = default_contact_consent
	...
	dataset_dict['inpp_access_limitations'] = convert_inpp_access_limitations(dataset_dict['inpp_access_limitations'])
	dataset_dict['inpp_use_constraints'] = convert_inpp_use_constraints(dataset_dict['inpp_use_constraints'])
	...
	if dataset_dict['inpp_use_constraints'] == 'open':
	...
	if dataset_dict['inpp_use_constraints'] in map_inpp_use_constraints_to_license_id:
		dataset_dict['license_id'] = map_inpp_use_constraints_to_license_id[dataset_dict['inpp_use_constraints']]
	else:
	...
	spatial_string = inppckan.inppckan_convert_bbox_to_geojson(
		dataset_dict['inpp_spatial_reference_system'],
		dataset_dict['inpp_north_latitude'],
		dataset_dict['inpp_south_latitude'],
		dataset_dict['inpp_west_longitude'],
		dataset_dict['inpp_east_longitude'])
	if spatial_string:
...
ckan_ip = open("/usr/lib/ckan/default/src/ckanext-inppschema/ckanext/inppschema/tools/ckan_ip.txt").read().replace('\n','')
api_key = open("/usr/lib/ckan/default/src/ckanext-inppschema/ckanext/inppschema/tools/ckan_api_key.txt").read().replace('\n','')

# Read CSV from theme plugin
topic_category_to_group_dict = inppckan.inppckan_create_mapping_topic_category_to_group()
